...
Table = $("#tblRegistros").DataTable({
	...
	{ "data": "FASE_PROYECTO" },
	{ "data": "UR" },
	{ "data": "NOMBRE_LIDER_PROYECTO" },
	{ "data": "NOMBRE_APROBO" },
	{
	    data: null,
	    className: "center",
	    defaultContent: '<a class="btn btn-xs btn-info editor_edit"><i class="glyphicon glyphicon-pencil"></i></a>'
	}
    ...
    $('#tblRegistros').on('click', 'a.editor_edit', function (e) {
        e.preventDefault();
		row = $(this).closest("tr");
		tdfip = row.find("td:nth-child(1)");
		console.log(tdfip);
		$(location).attr('href', base_url + 'index.php/captura/index/' + tdfip[0].innerHTML)
    } );
        ...
        if(r.length > 0){
	        table = '<tr class="tdTitulo"><th colspan="6"><center>Fólios encontrados: ' + r.length + ' <a onclick="tableToExcel()"><i class="glyphicon glyphicon-cloud-download"></i></a></center></th></tr>';
	     	for (i = 0; i < r.length; i++) {
				...
				table += '<tr><td>' + r[i]['ID_PROYECTO'] + '</td><td>'  + r[i]['FEC_REGISTRO'] + '</td><td>' + r[i]['ANIO'] + '</td><td>' + r[i]['FOLIO_PROYECTO'] + '</td><td> ' + r[i]['VERSION'] + '</td><td>' + r[i]['NOMBRE_PROYECTO'] + '</td><td>' + r[i]['FASE_PROYECTO'] + '</td><td>' + r[i]['UR'] + '</td><td>' + r[i]['NOMBRE_LIDER_PROYECTO']  + '</td><td>' + r[i]['NOMBRE_APROBO']  + '</td><td><center><a href="' + base_url + 'index.php/captura/index/' + r[i]['ID_PROYECTO'] +  '" class="btn btn-info btn-sm btnDetalle"><span class="glyphicon glyphicon-info-sign"></span></a></center></td></<tr>';
			}
			$("#tbodyRegistros").append(table);
        }
