...
class Ion_auth_model extends CI_Model
{
	...
	public function ldap_data(){
		 	return $this->db->select('*')
			                  ->where('ESTATUS', 1)
			                  ->get('T_LDAP_UR')->result_array();
	}

	public function ldap_update_password($ID_USUARIO,$PASSWORD){
		$this->db->update($this->tables['users'], array('PASSWORD' => $PASSWORD), array('ID_USUARIO' => $ID_USUARIO));
	}

	public function ldap_get_id_usuario($NOMBRE_USUARIO){
		 	return $this->db->select('ID_USUARIO')
			                  ->where('NOMBRE_USUARIO', $NOMBRE_USUARIO)
			                  ->get($this->tables['users'])->row('ID_USUARIO');
	}
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$query = $this->db->get_where($this->tables['groups'],array('NOMBRE_GRUPO' => $this->config->item('default_group', 'ion_auth')),1)->row();
		if( !isset($query->ID_GRUPO) && empty($groups) )
		{
		...
		$pid = getPropCat($this->tables['users'],'id');
		//$seq = getPropCat($this->tables['users'],'seq');
	  	//$nv = "$seq.NEXTVAL";
		//$this->db->set($pid, $nv, FALSE);
		$this->db->insert($this->tables['users'], $user_data);
		// Obtenemos el ultimo ID registrado
		//$id = $this->db->insert_id();
		$id = $this->db->select($pid)->from($this->tables['users'])->order_by($pid,'DESC')->limit(1)->get()->row($pid);
		// add in groups array if it doesn't exists and stop adding into default group if default group ids are set


		if( isset($default_group->ID_GRUPO) && empty($groups) )
		{
			$groups[] = $default_group->ID_GRUPO;
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', EMAIL, ID_USUARIO AS ID, PASSWORD, ACTIVE, LAST_LOGIN')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('ID_USUARIO', 'desc')
		                  ->get($this->tables['users']);
	...
	public function users($groups = NULL)
	{
		...
		if (isset($groups))
		{
			...
			{
				$this->db->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.ID_GRUPO', 'inner');
				$this->db->where_in($this->tables['groups'] . '.NOMBRE_GRUPO', $group_names);
			}
			if(!empty($group_ids))
			{
	...
	public function user($id = NULL)
	{
		...
		$this->limit(1);
		$this->order_by($this->tables['users'].'.ID_USUARIO', 'desc');
		$this->where($this->tables['users'].'.ID_USUARIO', $id);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as ID, '.$this->tables['groups'].'.NOMBRE_GRUPO, '.$this->tables['groups'].'.DESCRIPTION')
		                ->where($this->tables['users_groups'].'.'.$this->join['users'], $id)
		                ->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.ID_GRUPO')
	...
	public function add_to_group($group_ids, $user_id=false)
	{
		...
		$seq = getPropCat($this->tables['users_groups'],'seq');
		foreach ($group_ids as $group_id)
		{
		  	//$nv = "$seq.NEXTVAL";
			//$this->db->set('ID_USUARIO_GRUPOS', $nv, FALSE);
			if ($this->db->insert($this->tables['users_groups'], array( $this->join['groups'] => (float)$group_id, $this->join['users'] => (float)$user_id)))
			{
				if (isset($this->_cache_groups[$group_id])) {
					...
					$group_name = $group[0]->NOMBRE_GRUPO;
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$this->where($this->tables['groups'].'.ID_GRUPO', $id);
		}

		$this->limit(1);
		$this->order_by('ID_GRUPO', 'desc');
